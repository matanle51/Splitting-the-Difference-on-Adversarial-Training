...
from whitebox_attack import cwloss
...
def _pgd_blackbox(model_target,
    ...
    for _ in range(num_steps):
        ...
        with torch.enable_grad():
            if loss_type == 'ce':
                ...
                loss = cwloss(model_source(X_pgd), y)
        loss.backward()
    ...
    with torch.no_grad():
        pred = model_target(X_pgd, proj=True).data.max(1)[1]
...
def eval_adv_test_blackbox(model_target, model_source, device, test_loader, num_test_samples,
                           epsilon, step_size, num_attack_steps, loss_type='ce', num_classes=None):
    """
    ...
    for data, target in tqdm(test_loader):
        ...
        err_natural, err_robust = _pgd_blackbox(model_target, model_source, X, y,
                                                epsilon, num_attack_steps, step_size, loss_type, num_classes)
