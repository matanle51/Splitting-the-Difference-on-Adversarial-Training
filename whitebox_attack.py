...
def _pgd_whitebox(model,
    ...
    for _ in range(num_attack_steps):
        ...
        with torch.enable_grad():
            if loss_type == 'ce':
                loss = nn.CrossEntropyLoss()(model(X_pgd, proj), y)
            elif loss_type == 'cw':
                loss = cwloss(model(X_pgd, proj), y, num_classes=num_classes)
            else:
    ...
    with torch.no_grad():
        pred = model(X_pgd, proj=True).data.max(1)[1]
...
def eval_adv_test_whitebox(model, device, test_loader, num_test_samples, epsilon, step_size,
                           num_attack_steps, loss_type='ce', num_classes=None, proj=True):
    """
    ...
    for data, target in tqdm(test_loader):
        ...
        err_natural, err_robust = _pgd_whitebox(model, X, y,
                                                ...
                                                step_size=step_size, loss_type=loss_type, num_classes=num_classes, proj=proj)
